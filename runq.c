...
void generate(Transformer *transformer, Sampler *sampler, char *prompt, int steps) {
  char empty_prompt[] = "";
  if (prompt == NULL) {
    prompt = empty_prompt;
  }
  ...
  encode_sp(prompt, 1, 0, prompt_tokens, &num_prompt_tokens);
...
void chat(Transformer *transformer, Sampler *sampler, char *cli_user_prompt, char *cli_system_prompt, int steps) {
  ...
  while (pos < steps) {
	...
	*/
      ...
      sprintf(system_prompt, "%s%s%s", instruct_prompt_1, user_prompt, instruct_prompt_2);
      
      encode_sp(system_prompt, 0, 0, user_prompt_tokens, &num_user_prompt_tokens);
...
int main(int argc, char *argv[]) {
  ...
  char *tokenizer_path = "llm-jp-tokenizer-100k.ver3.0b1.model";
  ...
  if (strcmp(mode, "generate") == 0) {
    generate(&transformer, &sampler, prompt, steps);
  } else if (strcmp(mode, "chat") == 0) {
    chat(&transformer, &sampler, prompt, system_prompt, steps);
  } else {
